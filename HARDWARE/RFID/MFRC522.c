...
{ 
	...
	//配置PC8为输入模式
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_0;			//第8个引脚
	...
	//配置PC6 PC11为输出模式
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_12;			//第6 11个引脚
	...
	//配置PD6 PD7为输出模式
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_4|GPIO_Pin_14;			//第6 7个引脚
	GPIO_InitStructure.GPIO_Mode=GPIO_Mode_OUT;		//输出模式
	GPIO_InitStructure.GPIO_Speed=GPIO_High_Speed;		//引脚高速工作，收到指令立即工作；缺点：功耗高
	GPIO_InitStructure.GPIO_OType=GPIO_OType_PP;		//增加输出电流的能力
	GPIO_InitStructure.GPIO_PuPd=GPIO_PuPd_NOPULL;		//不需要上下拉电阻
	...
	//配置PD6 PD7为输出模式
	GPIO_InitStructure.GPIO_Pin=GPIO_Pin_7;			//第6 7个引脚
	GPIO_InitStructure.GPIO_Mode=GPIO_Mode_OUT;		//输出模式
	GPIO_InitStructure.GPIO_Speed=GPIO_High_Speed;		//引脚高速工作，收到指令立即工作；缺点：功耗高
	GPIO_InitStructure.GPIO_OType=GPIO_OType_PP;		//增加输出电流的能力
	GPIO_InitStructure.GPIO_PuPd=GPIO_PuPd_NOPULL;		//不需要上下拉电阻
	...
	//配置引脚的初始电平状态为高电平
	MFRC522_CS(1);
	PDout(4)=1;	//SPI 模式3，时钟线空闲的时候，必须为高电平
	PDout(14)=1;
	MFRC522_Rst(1);																						//启动SP3
}
